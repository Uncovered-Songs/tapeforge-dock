/* data.ts — rich, believable sample data for TapeForge Dock.
   Ported verbatim from the design's dock-data.js, typed for TS. This is mock
   data for the first UI pass; a real API will replace it later. */
...
export const systems: DockSystem[] = [
...
export const sessions: DockSession[] = [
...
export const eventStream: DockEvent[] = [
...
export const configChanges: ConfigChange[] = [
  { t: '19:00', who: 'M. Ndlovu', what: 'Set master bus loudness target', from: '—', to: '-23 LUFS' },
  { t: '20:11', who: 'auto · CORE', what: 'PTP grandmaster', from: 'GM-A (stage)', to: 'GM-B (FOH)' },
  { t: '20:34', who: 'M. Ndlovu', what: 'RIG · de-esser threshold (Lead Vox)', from: '-18 dB', to: '-22 dB' },
  { t: '21:05', who: 'auto · CORE', what: 'Buffer size (PORT-Stage)', from: '64', to: '128' },
]

export const crewRecs: CrewRec[] = [
...
export const crewThread: CrewTurn[] = [
  { role: 'user', text: 'Why did PORT-Stage disconnect during the Civic Theatre broadcast yesterday?' },
  {
    role: 'crew',
    text: "PORT-Stage didn't fully disconnect — it triggered a PTP clock failover at 20:11:14. The grandmaster offset climbed from 18µs to 41µs over ~30 min, then a 0.9ms jitter spike on stream 12 pushed the clock domain out of sync. CORE failed over to grandmaster B (FOH) and PTP re-locked within 48 seconds at 6µs.",
    cites: [
      { t: 'Session S-2040 · event 20:11:14', to: 'session' },
      { t: 'Doc · PTP & clocking', to: 'docs' },
    ],
    agents: ['Networking', 'Topology'],
  },
  { role: 'user', text: 'Has this happened before at Riverside Arena?' },
  {
    role: 'crew',
    text: 'Yes — 3 of the last 5 sessions at Riverside Arena (S-2041, S-2038, and one rehearsal) show PTP offset exceeding 40µs. The common factor is two grandmaster-capable PORT nodes with equal priority. I drafted a playbook recommending a fixed priority hierarchy.',
    cites: [
      { t: 'Knowledge · clock topology playbook', to: 'knowledge' },
      { t: '3 related sessions', to: 'sessions' },
    ],
    agents: ['Deployment History', 'Troubleshooting'],
  },
]

export const diagnostics: Diagnostics = {
  clockOffset: [12, 14, 13, 18, 22, 31, 41, 38, 9, 6, 7, 6],
  jitter: [0.2, 0.3, 0.25, 0.4, 0.5, 0.7, 0.9, 0.6, 0.3, 0.2, 0.2, 0.25],
  latency: [2.1, 2.0, 2.2, 2.1, 2.4, 2.3, 2.2, 2.1, 2.0, 2.1, 2.2, 2.1],
  packetLoss: [0, 0, 0, 0, 0.01, 0.03, 0.08, 0.02, 0, 0, 0, 0],
  incidents: [
